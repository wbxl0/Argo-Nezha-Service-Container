...
if [ ! -s /etc/supervisor/conf.d/damon.conf ]; then
  ...
  for PROXY_URL in "${GITHUB_PROXY[@]}"; do
    PROXY_STATUS_CODE=$(wget --server-response --spider --quiet --timeout=3 --tries=1 ${PROXY_URL}https://github.com/wbxl0/Argo-Nezha-Service-Container/raw/main/README.md 2>&1 | awk '/HTTP\//{last_field = $2} END {print last_field}')
...
EOF
    ...
    # AGENT_SECRET_KEY 可通过环境变量固定，避免容器重建时随机换 key 导致全部探针失联
    LOCAL_TOKEN=${AGENT_SECRET_KEY:-$(tr -dc 'A-Za-z0-9' </dev/urandom | head -c 32)}
...
EOF
  ...
  if [[ "$DASHBOARD_VERSION" =~ 0\.[0-9]{1,2}\.[0-9]{1,2}$ ]]; then
    ...
    wget -P ${WORK_DIR}/data/ ${GH_PROXY}https://github.com/wbxl0/Argo-Nezha-Service-Container/raw/main/sqlite.db
